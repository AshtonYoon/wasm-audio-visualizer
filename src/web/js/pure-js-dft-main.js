import { PureFFT } from "./pure-dft.js";
...
class PureJSDFTApp {
  ...
  async handleFileSelect(event) {
    ...
    try {
      ...
      console.log(
        `✓ 파싱 완료: ${file.name}, ${this.duration.toFixed(2)}초, ${
          this.sampleRate
        }Hz, ${this.channels}채널`
      );
  ...
  parseWAV(arrayBuffer) {
    ...
    while (offset < view.byteLength) {
      ...
      if (chunkId === "fmt ") {
        ...
        console.log(
          `WAV 포맷: ${channels}채널, ${sampleRate}Hz, ${bitsPerSample}bit`
        );
          ...
          if (dataChunkId === "data") {
            ...
            for (let i = 0; i < numSamples; i++) {
              ...
              for (let ch = 0; ch < channels; ch++) {
                const byteOffset =
                  dataStart + (i * channels + ch) * bytesPerSample;
                let sample = 0;
